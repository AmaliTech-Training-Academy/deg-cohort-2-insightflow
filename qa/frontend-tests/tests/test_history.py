...
class TestIngestionHistoryPage:
    ...
    def test_new_upload_button_visible(self, auth_page: Page, base_url: str):
        self._go(auth_page, base_url)
        expect(
            auth_page.get_by_role("link", name="New Upload", exact=True)
        ).to_be_visible()

    def test_table_columns_render(self, auth_page: Page, base_url: str):
        self._go(auth_page, base_url)
        for col in ["File", "Source", "Status", "Records", "Started"]:
            expect(auth_page.locator(f"th:has-text('{col}')")).to_be_visible(
                timeout=10000
            )
    ...
    def test_new_upload_button_navigates_to_upload(
    ...
    def test_sidebar_shows_ingestion_history_as_active(
        self, auth_page: Page, base_url: str
    ):
        self._go(auth_page, base_url)
